...
class TestFilesController:
    ...
    def test_upload_file_too_large(self, client, auth_headers):
        ...
        with patch("app.support.files_uploader.FilesUploader.perform") as mock_perform:
            ...
            data = {"file": (BytesIO(b"large file content"), "large.txt")}
    ...
    def test_upload_file_unauthorized(self, client):
        ...
        response = client.post(
            "/api/files/upload-files", data=data, content_type="multipart/form-data"
        )
    ...
    def test_get_presigned_url_success(self, client, auth_headers, mock_s3):
        ...
        with patch(
            "app.controllers.api.v1.files_controller.generate_presigned_s3_url"
        ) as mock_generate:
            mock_generate.return_value = "https://test-presigned-url.com"

            response = client.get(
                "/api/files/presigned_url?file_type=user_file&file_name=test.jpg",
                headers=auth_headers,
            )
    ...
    def test_get_presigned_url_missing_params(self, client, auth_headers):
        ...
        # Missing file_type
        response = client.get(
            "/api/files/presigned_url?file_name=test.jpg", headers=auth_headers
        )
        ...
        # Missing file_name
        response = client.get(
            "/api/files/presigned_url?file_type=image", headers=auth_headers
        )
    ...
    def test_get_presigned_url_unauthorized(self, client):
        ...
        response = client.get(
            "/api/files/presigned_url?file_type=image&file_name=test.jpg"
        )
