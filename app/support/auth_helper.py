        ...
        def decorated(*args, **kwargs):
            ...
            if token:
                try:
                    ...
                    if current_user:
                        valid_access = validate_user_permission(allowed_feature, current_user['role'])
                        if not valid_access:
                            ...
                            session['expire_at'] = int(time.time()) + int(Config.SESSION_TIME)

                    else:
                        error_msg = "User is inactive or does not exit. Please contact your administrator!"

                except Exception as e:
                    error_msg = "Something went wrong. Please contact your administrator!"

            else:
                error_msg = "Session timeout. Please visit application to connect to tool!"

            # checking errors
            if error_msg:
                return render_template("web/login.html", title="Api Base Tool", message=error_msg)
