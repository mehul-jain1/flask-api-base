...
class S3_Client(object):
    ...
    def put_object(self, filepath, filename, file_obj):
        try:
            response = self.s3_client.put_object(Bucket=S3_BUCKET, Key=filepath, Body=file_obj)
        except Exception as e:
            logging.error(e)
            return None

        return { "status": response['ResponseMetadata']['HTTPStatusCode'], "filename": filename }

    def generate_presigned_url(self, filepath, expiration=3600):
        try:
            response = self.s3_client.generate_presigned_url('get_object', Params={'Bucket': S3_BUCKET, 'Key': filepath, 'ResponseContentDisposition': 'attachment'}, ExpiresIn=expiration)
        except Exception as e:
            logging.error(e)
            return None
