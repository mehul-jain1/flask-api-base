...
def sample_user_data():
    ...
    return {"name": "Test User", "email": "test@example.com", "role": "manager"}
...
def _create_test_data():
    ...
    try:
        ...
        admin_user_access = FeatureRole(
            feature_id=user_feature.id, role_id=admin_role.id
        )
        admin_file_access = FeatureRole(
            feature_id=file_feature.id, role_id=admin_role.id
        )
        ...
        # Create test users
        admin_user = User(
            name="Admin User", email="admin@test.com", role_id=admin_role.id
        )

        test_user = User(
            name="Test User", email="test@test.com", role_id=manager_role.id
        )
...
def mock_s3():
    ...
    with mock.patch("app.support.s3_helper.put_object_to_s3") as mock_put:
        ...
        with mock.patch(
            "app.support.s3_helper.generate_presigned_s3_url"
        ) as mock_presigned:
            mock_presigned.return_value = "https://mock-presigned-url.com"
            yield {"put_object": mock_put, "presigned_url": mock_presigned}
