...
class TestUsersController:
    ...
    def test_create_user_success(
        self, client, auth_headers, sample_user_data, mock_celery
    ):
        ...
        with patch("app.services.users.saver.UserSaver.save") as mock_save:
            ...
            response = client.post(
                "/api/users", json=sample_user_data, headers=auth_headers
            )
    ...
    def test_create_user_validation_error(self, client, auth_headers):
        ...
        response = client.post(
            "/api/users", json=invalid_user_data, headers=auth_headers
        )
        data = response.get_json()
    ...
    def test_create_user_save_failure(self, client, auth_headers, sample_user_data):
        ...
        with patch(
            "app.controllers.api.v1.users_controller.UserSaver"
        ) as mock_user_saver_class:
    ...
    def test_get_all_users_with_pagination(self, client, auth_headers):
        ...
        response = client.get(
            "/api/users?pageNumber=1&pageSize=5", headers=auth_headers
        )
        data = response.get_json()
    ...
    def test_create_user_schema_validation_error(
        self, mock_validate, client, auth_headers
    ):
    ...
    def test_create_user_data_validation_error(
        self, mock_validate, client, auth_headers, sample_user_data
    ):
